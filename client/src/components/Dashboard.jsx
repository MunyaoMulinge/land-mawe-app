...
export default function Dashboard() {
  ...
  return (
    <div>
      ...
      {/* Fleet Overview */}
      <h3 style={{ marginBottom: '0.5rem', color: '#333' }}>🚛 Fleet Overview</h3>
      ...
      {/* Financial Overview */}
      <h3 style={{ marginBottom: '0.5rem', color: '#333' }}>💰 Financial Overview</h3>
      <div className="stats-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)', marginBottom: '1.5rem' }}>
        <div className="stat-card" style={{ borderLeft: '4px solid #28a745' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem', marginBottom: '1.5rem' }}>
        ...
        <div className="card">
          ...
          {jobCardStats ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem' }}>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#e9ecef', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{jobCardStats.draft || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#666' }}>Draft</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#fff3cd', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#856404' }}>{jobCardStats.pending_approval || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#856404' }}>Pending</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#cce5ff', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#004085' }}>{jobCardStats.approved || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#004085' }}>Approved</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#d1ecf1', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#0c5460' }}>{jobCardStats.departed || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#0c5460' }}>On Trip</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#d4edda', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#155724' }}>{jobCardStats.completed || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#155724' }}>Completed</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#f8f9fa', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{jobCardStats.total || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#666' }}>Total</div>
              </div>
            </div>
          ) : <p style={{ color: '#666' }}>No job card data</p>}
        </div>
        ...
        <div className="card">
          ...
          {complianceStats ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.5rem' }}>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#d4edda', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#155724' }}>{complianceStats.active || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#155724' }}>Active</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#fff3cd', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#856404' }}>{complianceStats.expiring_soon || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#856404' }}>Expiring Soon</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#f8d7da', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#721c24' }}>{complianceStats.expired || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#721c24' }}>Expired</div>
              </div>
            </div>
          ) : <p style={{ color: '#666' }}>No compliance data</p>}
          {complianceStats?.mandatory_expired > 0 && (
            <div style={{ marginTop: '0.5rem', padding: '0.5rem', background: '#f8d7da', borderRadius: '4px', color: '#721c24', fontSize: '0.85rem' }}>
            ...
            <>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem', marginBottom: '1rem' }}>
                <div style={{ textAlign: 'center', padding: '0.5rem', background: '#fff3cd', borderRadius: '6px' }}>
                  ...
                  <div style={{ fontSize: '0.8rem', color: '#856404' }}>Scheduled</div>
                </div>
                <div style={{ textAlign: 'center', padding: '0.5rem', background: '#cce5ff', borderRadius: '6px' }}>
                  <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#004085' }}>{maintenanceStats.in_progress || 0}</div>
                  <div style={{ fontSize: '0.8rem', color: '#004085' }}>In Progress</div>
                </div>
              </div>
              {upcomingMaintenance.overdue?.length > 0 && (
                <div style={{ padding: '0.5rem', background: '#f8d7da', borderRadius: '4px', color: '#721c24', fontSize: '0.85rem' }}>
                  ⚠️ {upcomingMaintenance.overdue.length} overdue maintenance task(s)
                </div>
              )}
            </>
          ) : <p style={{ color: '#666' }}>No maintenance data</p>}
        </div>
        ...
        <div className="card">
          ...
          {fuelStats ? (
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.5rem' }}>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#e9ecef', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{fuelStats.total_records || 0}</div>
                <div style={{ fontSize: '0.8rem', color: '#666' }}>Refills</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#e9ecef', borderRadius: '6px' }}>
                <div style={{ fontSize: '1.2rem', fontWeight: 'bold' }}>{(fuelStats.total_liters || 0).toLocaleString()}L</div>
                <div style={{ fontSize: '0.8rem', color: '#666' }}>Total Liters</div>
              </div>
              <div style={{ textAlign: 'center', padding: '0.5rem', background: '#fff3cd', borderRadius: '6px', gridColumn: 'span 2' }}>
                <div style={{ fontSize: '1rem', fontWeight: 'bold', color: '#856404' }}>
                  Avg: {formatCurrency(fuelStats.avg_cost_per_liter || 0)}/L
                </div>
              </div>
            </div>
          ) : <p style={{ color: '#666' }}>No fuel data</p>}
        </div>
      </div>
      ...
      <div className="card" style={{ marginBottom: '1.5rem' }}>
        ...
        {invoiceStats ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: '0.5rem' }}>
            <div style={{ textAlign: 'center', padding: '0.5rem', background: '#e9ecef', borderRadius: '6px' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{invoiceStats.draft || 0}</div>
              <div style={{ fontSize: '0.8rem', color: '#666' }}>Draft</div>
            </div>
            <div style={{ textAlign: 'center', padding: '0.5rem', background: '#cce5ff', borderRadius: '6px' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#004085' }}>{invoiceStats.sent || 0}</div>
              <div style={{ fontSize: '0.8rem', color: '#004085' }}>Sent</div>
            </div>
            <div style={{ textAlign: 'center', padding: '0.5rem', background: '#d4edda', borderRadius: '6px' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#155724' }}>{invoiceStats.paid || 0}</div>
              <div style={{ fontSize: '0.8rem', color: '#155724' }}>Paid</div>
            </div>
            <div style={{ textAlign: 'center', padding: '0.5rem', background: '#f8d7da', borderRadius: '6px' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold', color: '#721c24' }}>{invoiceStats.overdue || 0}</div>
              <div style={{ fontSize: '0.8rem', color: '#721c24' }}>Overdue</div>
            </div>
            <div style={{ textAlign: 'center', padding: '0.5rem', background: '#f8f9fa', borderRadius: '6px' }}>
              <div style={{ fontSize: '1.5rem', fontWeight: 'bold' }}>{invoiceStats.total || 0}</div>
              <div style={{ fontSize: '0.8rem', color: '#666' }}>Total</div>
            </div>
          </div>
        ) : <p style={{ color: '#666' }}>No invoice data</p>}
      </div>
